...
fn experience(
    ...
    if !is_dev {
        download_file(&format!("{}/launcher/api/redirect", BACKEND_URL), &game_dll)
    ...
    let is_season12 = version.starts_with("12.");
    ...
    if !is_season12 {
        fort_args.push("-caldera=eyJhbGciOiJFUzI1NiIsInR5cCI6IkpXVCJ9.eyJhY2NvdW50X2lkIjoiOWM1MDY1MTEwYzdhNGQ3MDk1ODYyZGE1ZWU4MTU5NjIiLCJnZW5lcmF0ZWQiOjE2NjczMjMxMzYsImNhbGRlcmFHdWlkIjoiOGFkOTEyZGYtZTcwMy00NmJhLWE2ZjQtOWM3ZGE4NjMzN2FmIiwiYWNQcm92aWRlciI6IkJhdHRsRXllIiwibm90ZXMiOiIiLCJmYWxsYmFjayI6ZmFsc2V9.m6wk19aqhW3yGrPcR3OqNwZJbwF3Bv5Dv9p-elzJwG670Xn0yb2Y2CCvkKzR9XNDX6mzgCTlo2SIpiK1Du2xNA");
    }

    fort_args.push("-fromfl=eac");
    fort_args.push("-fltoken=919348d6add4c4c7c7507e61");
    ...
    let _ac = std::process::Command::new(&fnac)
        ...
        .stdout(std::process::Stdio::piped())
        .spawn()
        .map_err(|e| format!("Failed to start BE: {}", e))?;
...
pub fn run() {
    ...
    tauri::Builder
        ...
        // .plugin(tauri_plugin_updater::Builder::new().build()) // disabled: requires https endpoint
        ...
        .setup(|app| {
            ...
            std::thread::spawn(move || {
                if let Err(e) = tauri_plugin_deep_link::register("splash", move |request| {
                    ...
                    let _ = window_clone.emit("deep-link", request);
                }) {
